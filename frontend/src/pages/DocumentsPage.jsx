...
const DocumentsPage = () => {
  ...
  const DocumentHeader = ({ title }) => (
    <div className="text-center border-b-2 border-slate-300 pb-6 mb-8">
      ...
      <h2 className="font-heading font-bold text-xl text-slate-900 uppercase mt-4">{title}</h2>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={fetchData} variant="outline" className="border-slate-200 text-swiss-text-secondary hover:text-slate-900 rounded-sm">
            <RefreshCw className={`w-4 h-4 mr-2 ${loading ? 'animate-spin' : ''}`} />Refresh
          </Button>
          <Button onClick={handlePrint} className="bg-swiss-red hover:bg-swiss-red-hover text-white rounded-sm">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-swiss-bg-paper border border-slate-200 p-1 rounded-sm h-auto flex-wrap gap-1">
          {documentTypes.map((doc) => (
            <TabsTrigger key={doc.id} value={doc.id} className="data-[state=active]:bg-swiss-red data-[state=active]:text-white text-swiss-text-secondary rounded-sm px-3 py-2 text-xs">
              ...
              <div ref={printRef} className="p-6">
                    ...
                    <tbody>
                      {transactions.length === 0 ? <tr><td colSpan="5" className="text-center py-8 text-swiss-text-muted">No transactions</td></tr> :
                        transactions.map((tx, i) => (
                          <tr key={tx.id || i} className="border-b border-slate-100">
                            ...
                            <td className="py-3 px-2 text-slate-900 text-xs">{tx.description}</td>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    {balances.map((bal) => (
                      <div key={bal.currency} className="bg-swiss-bg-subtle p-4 rounded-sm">
                        ...
                        <p className="font-mono text-lg text-slate-900">{formatCurrency(bal.balance, bal.currency)}</p>
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-6">
                        <div className="bg-swiss-bg-subtle p-4 rounded-sm">
                          ...
                          <p className="font-mono text-slate-900">{transactions.filter(t => t.amount < 0)[0].reference}</p>
                        </div>
                        <div className="bg-swiss-bg-subtle p-4 rounded-sm">
                      ...
                      <div className="bg-swiss-bg-subtle p-4 rounded-sm">
                        ...
                        <p className="text-slate-900">{transactions.filter(t => t.amount < 0)[0].description}</p>
                      </div>
                    </div>
                ...
                <TabsContent value="remittance-advice" className="mt-0">
                  ...
                  {transfers.slice(0, 5).map((t, i) => (
                    <div key={t.id || i} className="mb-4 p-4 bg-swiss-bg-subtle rounded-sm">
                      <div className="flex justify-between"><div><p className="text-slate-900 font-medium">{t.beneficiary_name}</p><p className="font-mono text-xs text-swiss-text-muted">{t.beneficiary_iban}</p></div>
                        <p className="font-mono text-xl text-slate-900">{formatCurrency(t.amount, t.currency)}</p></div>
                ...
                <TabsContent value="balance-sheet" className="mt-0">
                  ...
                  <table className="w-full text-sm mb-6">
                    ...
                    <tbody>
                      {balances.map((bal) => (
                        <tr key={bal.currency} className="border-b border-slate-100">
                          <td className="py-3 px-2 text-slate-900">{bal.currency}</td>
                    ...
                    <tbody>
                      {[{n:'SWIFT Gateway',s:'ONLINE',u:'99.99%'},{n:'TARGET2',s:'ONLINE',u:'99.97%'},{n:'Core Banking',s:'ONLINE',u:'100%'},{n:'Database',s:'ONLINE',u:'99.99%'},{n:'API Gateway',s:'ONLINE',u:'99.98%'}].map((srv,i) => (
                        <tr key={i} className="border-b border-slate-100">
                          <td className="py-3 px-2 text-slate-900">{srv.n}</td>
                ...
                <TabsContent value="alliance-report" className="mt-0">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="bg-swiss-bg-subtle p-4 rounded-sm"><p className="text-swiss-text-muted text-xs">Total Messages</p><p className="font-mono text-2xl text-slate-900">{transfers.length}</p></div>
                    ...
                    <tbody>
                      {transfers.slice(0, 10).map((t, i) => (
                        <tr key={t.id || i} className="border-b border-slate-100">
                          ...
                          <td className="py-3 px-2 text-right font-mono text-xs text-slate-900">{formatCurrency(t.amount, t.currency)}</td>
                  ...
                  {transfers.slice(0, 5).map((t, i) => (
                    <div key={t.id || i} className="mb-4 p-4 bg-swiss-bg-subtle rounded-sm">
                      <div className="flex justify-between items-start mb-4">
                        <div><p className="text-swiss-text-muted text-xs">Tracking ID</p><p className="font-mono text-sm text-slate-900">{t.tracking_id}</p></div>
                        <Badge className="bg-green-500/20 text-green-400 rounded-sm">{t.status}</Badge>
                      </div>
                      <div className="grid grid-cols-4 gap-4 text-xs">
                        <div><p className="text-swiss-text-muted">Beneficiary</p><p className="text-slate-900">{t.beneficiary_name}</p></div>
                        <div><p className="text-swiss-text-muted">BIC</p><p className="font-mono text-slate-900">{t.beneficiary_swift}</p></div>
                        <div><p className="text-swiss-text-muted">Amount</p><p className="font-mono text-slate-900">{formatCurrency(t.amount, t.currency)}</p></div>
                        <div><p className="text-swiss-text-muted">Date</p><p className="text-slate-900">{formatDate(t.created_at)}</p></div>
                      </div>
                      <div className="mt-4 pt-4 border-t border-slate-200 flex items-center gap-2 text-xs">
